...
### 9. If SavedDataForLoad.json file already exists but is completely empty, the software shall still be able to start up the program and load all the orders that still needs to be processed
...
### 12. The software shall only allow order type "delivery" to be able to use the "Delivery Status" bar and update its status
